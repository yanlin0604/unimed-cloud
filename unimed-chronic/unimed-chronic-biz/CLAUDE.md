...
- **2026-08-18** - 遗留项收口：全仓 mock 清零（医生端 契约/诊疗/分组/消息/工作台/我的，管理端 转诊/知情同意/OCR/调档/筛查/区域/ICD 接真实 API；设备/通知模板/自定义分组/临床路径改诚实空态）；线上菜单落库 183 行且权限码 162/162 对齐后端（原 162 个死按钮清零）；新建"慢病医生"角色 + 隐藏权限载体菜单承载 36 个 `chronic:doctor:*`（原 0 承载必 403）+ 医生账号 doctor2001~2008；修复 `DoctorPatientController.page` 越权（未按登录医生过滤）、签约不写 SIGN 时间线、`ChScreeningBatchServiceImpl` 漏 doctorUserId 过滤；新增 `FollowupOverdueRefresher`（Redis 节流懒刷新，SnailJob 未部署时逾期状态仍正确）；新增 `GET /doctor/screening/batches`；部署清单见 `script/sql/update/CHRONIC-DEPLOY.md`，校验脚本 `_check_api_contract.py`（三端调用点 0 断点）与 `_check_perm_coverage.py`
