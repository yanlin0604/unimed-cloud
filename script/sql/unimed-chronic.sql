...
CREATE TABLE `ch_followup_task` (
  ...
  `del_flag`          CHAR(1)       DEFAULT '0' COMMENT '删除标志(0存在 1删除)',
  PRIMARY KEY (`task_id`),
  UNIQUE KEY `uk_plan_round_patient` (`plan_id`, `task_round`, `patient_id`),
...
CREATE TABLE `ch_health_metric_record` (
  ...
  `metric_value`       VARCHAR(500)  DEFAULT NULL COMMENT '指标值（简单指标为字符串数字，血压等复合指标为JSON）',
...
CREATE TABLE `ch_warning_event` (
  ...
  `warning_value`    VARCHAR(500)  DEFAULT NULL COMMENT '预警值（同指标值格式）',
...
CREATE TABLE `ch_referral_record` (
  ...
  `referral_user_id`  BIGINT        DEFAULT NULL COMMENT '转诊人用户ID',
  `create_dept`    BIGINT        DEFAULT NULL COMMENT '创建部门',
  `tenant_id`      BIGINT        DEFAULT NULL COMMENT '租户ID',
  `create_by`         BIGINT        DEFAULT NULL COMMENT '创建者',
  `create_time`       DATETIME      DEFAULT NULL COMMENT '创建时间',
...
CREATE TABLE `ch_archive_share_apply` (
  ...
  `approval_status`     VARCHAR(20)   DEFAULT NULL COMMENT '审批状态(PENDING/APPROVING/APPROVED/REJECTED/WITHDRAWN)',
...
CREATE TABLE `ch_encounter_record` (
  `id`                    BIGINT        NOT NULL AUTO_INCREMENT COMMENT '主键',
  `patient_id`            BIGINT        DEFAULT NULL COMMENT '患者ID',
  `disease_code`          VARCHAR(32)   DEFAULT NULL COMMENT '病种编码',
  `encounter_type`        VARCHAR(20)  DEFAULT NULL COMMENT '就诊类型(INITIAL/FOLLOWUP)',
  `encounter_time`         DATETIME      DEFAULT NULL COMMENT '就诊时间',
  `complaint`             TEXT         DEFAULT NULL COMMENT '主诉',
  `present_history`      TEXT         DEFAULT NULL COMMENT '现病史',
  `physical_exam_summary` TEXT         DEFAULT NULL COMMENT '体格检查摘要',
  `auxiliary_exam_summary` TEXT       DEFAULT NULL COMMENT '辅助检查摘要',
  `treatment_plan`         TEXT         DEFAULT NULL COMMENT '处理方案',
  `revisit_advice`        TEXT         DEFAULT NULL COMMENT '复诊建议',
  `medication_snapshot`   JSON         DEFAULT NULL COMMENT '当前用药快照',
  `risk_factor_snapshot` JSON         DEFAULT NULL COMMENT '风险因素快照',
  `source_type`          VARCHAR(20)  DEFAULT NULL COMMENT '来源类型(DOCTOR/ADMIN/HIS)',
  `source_biz_no`        VARCHAR(50)  DEFAULT NULL COMMENT '外部单号或门诊号',
  `submit_status`        VARCHAR(20)  DEFAULT 'DRAFT' COMMENT '提交状态(DRAFT/SUBMITTED)',
  ...
  `del_flag`             CHAR(1)      DEFAULT '0' COMMENT '删除标志(0存在 1删除)',
  PRIMARY KEY (`id`),
  INDEX `idx_er_patient_id` (`patient_id`),
  INDEX `idx_er_encounter_time` (`encounter_time`),
  INDEX `idx_er_source_biz_no` (`source_biz_no`),
  INDEX `idx_er_tenant_id` (`tenant_id`)
...
CREATE TABLE `ch_encounter_diagnosis` (
  `id`                  BIGINT        NOT NULL AUTO_INCREMENT COMMENT '主键',
  `encounter_id`         BIGINT        DEFAULT NULL COMMENT '诊疗记录ID',
  `patient_id`          BIGINT        DEFAULT NULL COMMENT '患者ID',
  `diagnosis_type`       VARCHAR(20)  DEFAULT NULL COMMENT '诊断类型(PRIMARY/SECONDARY)',
  `diagnosis_code`       VARCHAR(20)  DEFAULT NULL COMMENT 'ICD编码',
  `diagnosis_name`      VARCHAR(100) DEFAULT NULL COMMENT '诊断名称',
  `diagnosis_basis`     TEXT         DEFAULT NULL COMMENT '诊断依据',
  `risk_factor_code`    VARCHAR(32)  DEFAULT NULL COMMENT '风险因素编码',
  `risk_factor_name`    VARCHAR(100) DEFAULT NULL COMMENT '风险因素名称',
  `complication_flag`    CHAR(1)       DEFAULT 'N' COMMENT '是否并发症(Y/N)',
  `create_dept`         BIGINT       DEFAULT NULL COMMENT '创建部门',
  `tenant_id`           BIGINT       DEFAULT NULL COMMENT '租户ID',
  `create_by`           BIGINT       DEFAULT NULL COMMENT '创建者',
  `create_time`         DATETIME     DEFAULT NULL COMMENT '创建时间',
  ...
  `del_flag`            CHAR(1)      DEFAULT '0' COMMENT '删除标志(0存在 1删除)',
  PRIMARY KEY (`id`),
  INDEX `idx_ed_encounter_id` (`encounter_id`),
