...
--
-- trigger_type=1 表示 CRON；job_status=1 开启；task_type=1 集群；route_key=4 一致性哈希
-- 幂等：按 job_name 先删后插
-- ============================================================
...
('dev', 'unimed_group', '慢病-随访任务生成', NULL, 1, UNIX_TIMESTAMP() * 1000, 1, 1, 4, 1,
 'followupTaskGenJob', 1, '0 0 1 * * ?', 1, 300, 3, 1, 60, 0, 0, '', 1, '', '按随访计划周期生成下一轮随访任务', '', 0, NOW(), NOW()),

('dev', 'unimed_group', '慢病-签约SLA检查', NULL, 1, UNIX_TIMESTAMP() * 1000, 1, 1, 4, 1,
 'contractSlaCheckJob', 1, '0 0 2 * * ?', 1, 300, 3, 1, 60, 0, 0, '', 1, '', '签约履约项超期检查与提醒', '', 0, NOW(), NOW()),

('dev', 'unimed_group', '慢病-随访到期提醒', NULL, 1, UNIX_TIMESTAMP() * 1000, 1, 1, 4, 1,
 'followupRemindJob', 1, '0 0 8 * * ?', 1, 300, 3, 1, 60, 0, 0, '', 1, '', '逾期任务置OVERDUE、临期任务置REMINDING并通知执行医生', '', 0, NOW(), NOW()),

('dev', 'unimed_group', '慢病-统计日报', NULL, 1, UNIX_TIMESTAMP() * 1000, 1, 1, 4, 1,
 'statDailyJob', 1, '0 50 23 * * ?', 1, 600, 3, 1, 60, 0, 0, '', 1, '', '区域/随访/预警统计日表汇总', '', 0, NOW(), NOW());
